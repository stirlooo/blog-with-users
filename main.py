...
def register():
    ...
    if register_form.validate_on_submit():
        ...
        hash_and_salted_password=werkzeug.security.generate_password_hash(register_form.password.data, method='pbkdf2:sha256', salt_length=8)
        new_user = User(email=email, password=hash_and_salted_password, name=register_form.name.data)
...
def login():
    ...
    if login_form.validate_on_submit():
        ...
        if not user:
            ...
            login_user(user)
            print('Вы залогинились с имэйлом: ', email)
            return redirect(url_for('get_all_posts'))
